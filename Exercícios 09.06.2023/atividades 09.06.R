...
title(main="Função densidade probabilidade reparametrizada", xlab = "y", ylab="Densidade de probabilidade" )
...
negativo_ll_chen <- function(y, parametros){
  ...
  lambda <- parametros[1]
  delta <- parametros[2]
  log_verossimilhanca<- n*log(delta) + sum((lambda-1)*log(y)) + delta*(sum(1-exp(y^lambda)))+ sum(y^lambda)
  return (-log_verossimilhanca)
}
estimacao <- optim(par=c(2,2), fn=negativo_ll_chen, y=random_chen1, method ="BFGS", hessian =TRUE)
estimacao
